...
describe("unlinkObservable", () => {
  ...
  it("errors on unlink issue", async done => {
    ...
    fs.unlink.mockImplementation((path: any, callback: (arg0: { message: string; }) => void) =>
  ...
  it("completes and calls fs.existsSync, fs.unlink", async () => {
    expect.assertions(2);
    fs.existsSync.mockImplementation(() => true);
    fs.unlink.mockImplementation((path: any, callback: () => void) => callback());
...
describe("readdirObservable", () => {
  ...
  it("lists a directory with ✨  Observables ✨", async () => {
    expect.assertions(2);
    fs.readdir.mockImplementation((path: any, callback: (arg0: any, arg1: string[]) => void) => {
  ...
  it("handles errors listing directories, passes it back directly", async done => {
    expect.assertions(2);
    fs.readdir.mockImplementation((path: any, callback: (arg0: Error) => void) => {
