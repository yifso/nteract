...
export const kernelSpecs$ = from(kernelspecs.findAll()).pipe(
  map(
    (specs) =>
      ({
        ...specs,
...
export type SetKernelspecs = Action<
  typeof SET_KERNELSPECS,
  { kernelSpecs: Kernelspecs }
>;
export const setKernelspecs = makeActionFunction<SetKernelspecs>(
  SET_KERNELSPECS
);

kernelSpecs$
  .pipe(filter((kernelSpecs) => Object.keys(kernelSpecs).length > 0))
  .subscribe((kernelSpecs) =>
